...
%\usepackage{emoji}				% For using emojis (enable it when able to install lib)
...
\begin{document}
	...
	\section*{Préface}
	\thispagestyle{plain}
	\guillemotleft~\textit{C'est une blague}~?~\guillemotright\\~\rightline{--- La réaction de ma sœur en voyant ce recueil.} \\ \\
	...
	Avec 450 blagues\footnote{Et seulement 383 notes de bas de page~!}% Plus des blagues qui n'ont pas été publiées : si vous lisez cela, vous pourrez y accéder !
	...
	\begin{enumerate}
		...
		%\item C'est un stagiaire en développement qui utilise Git pour la première fois. Il veut récupérer la dernière version de la branche principale du projet. Donc on lui dit de faire un \textbf{git pull} mais rien n'y fait, ça ne marche pas. Au bout de longues minutes, son tuteur (manifestement excédé) jette un œil sur son PC et s'aperçoit qu'il a tapé \textbf{git (chicken)} % Unable to add this joke until I am able to add emoji of a chicken...

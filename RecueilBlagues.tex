...
% If you use TexStudio in English, your labels will be in English, even if your doc is in French.
...
\begin{document}
    ...
    % Cover pages -- Remove them if (#pages % 4 == 0)
    %\
    %\newpage
    %\thispagestyle{empty}
    %\
	...
	\section*{Préface}
	\thispagestyle{plain}
	\guillemotleft~\textit{C'est une blague}~?~\guillemotright\\~\rightline{--- Une réaction normale en voyant ce recueil.} \\ \\
	...
	Ce recueil de blagues vous donnera le sourire dès le premier calembour (ou vous rassurera sur votre niveau de blagues) et vous permettra de faire rire vos voisins~! \\
	Avec 463 blagues\footnote{Et seulement 389 notes de bas de page~!}% Plus des blagues qui n'ont pas été publiées : si vous lisez cela, vous pourrez y accéder !
	...
	\begin{enumerate}
		...
		%\item Comment reconnaît-on une orange qui prend tout son temps~? C’est celle qui n’est pas pressée\footnote{Mais si l’orange est pressée, elle doit être fatiguée et pour cause~: elle n’a plus de jus~!}. \\\textit{\underline{Variante}~: Marche aussi avec un citron.} %Redite avec une autre blague
		...
		%\item Si un individu est, étymologiquement, un être indivisible, est-ce que Louis XVI est un individu~? Marche aussi avec Marie-Antoinette.
		...
		% \item Pourquoi la septième lettre de l'alphabet est intelligente~? Parce qu'elle est un G\footnote{Ou ingé[nieure]. Et en tant qu'ingé, on n'est pas si bête !}.
        ...
        \item Les dinosaures ne font pas de simples retour d'expérience~: ils font des T-REx\footnote{Un REX ou RETEX est un retour d'expérience (fréquemment utilisé après des réussites ou des échecs d'un projet) afin d'en tirer des leçons à apprendre et s'améliorer. En l'occurence, un T-Rex aura de quoi débriefer après la crise du Crétacé-Tertiaire.}.
        \item Si Batman\footnote{Oui, c'est le sujet de plein de blagues mais je n'ai pas le choix~!} s'habille toujours en noir, peut-on dire que c'est un sombre héros\footnote{Mais il n'en porte pas. Quelle ironie~!}~?
        \item Que font deux caractères d'imprimerie dans une salle de muscu~? Ils soulèvent de la \textit{fonte}\footnote{Une fonte définit un ensemble de caractères typographiques partageant un même style et une même apparence (comme une police de caractères). Et la fonte, ce sont aussi les poids mis sur une haltère que l'on soulève. Ne les critiquez pas, ils ont mauvais caractère~!}.
        \item Quel postier a le plus de chance de gagner au loto~? Le facteur chance.
        \item Que dit un cumulonimbus lorsqu'un avion le traverse~? \guillemetleft~Rien à foudre\footnote{Faut dire, ça peut secouer dans l'avion alors que le nuage... et bien, il est dans son nuage.}~!~\guillemetright
        \item Quel est le comble pour un fakir~? De ne jamais traverser sur le passage clouté\footnote{Vu que les fakirs sont réputés pour ne pas ressentir de douleur (lors de la méditation) lorsqu'ils sont assis ou couchés sur \sout{cet engin de torture} une planche pleine de clous}.
        \item Qu'est-ce que deux canards qui se rencontrent de manière fortuite ? Une coincoincidence\footnote{Ça vous en bouche un coin-coin \emoji{duck}~?}~!
        \item Pourquoi les annuaires n'ont-elles pas besoin de se maquiller ? Parce qu'elles ont déjà un bottin\footnote{Synonyme pour un annuaire papier. Ceux nés après 2005 n'auront pas connu cette belle (?) époque -- non mais je parle comme un vieux~!}~!
        \item Pourquoi il ne faut pas appeler un pilote d'avion pendant son service~? Sinon, il décroche\footnote{Un avion qui décroche (\textit{stall}) est une situation à éviter car sa portance ne lui permet plus de rester en altitude (ex : trop faible vitesse par rapport au vent, angle d'incidence trop élevé (le nez est trop haut), etc). Donc il tombe jusqu'à ce que ces problèmes soient réglés.}~!
        \item Pourquoi Iron Man aura peu de chances d'aller au paradis ? Parce qu'il est Enfer\footnote{Iron Man peut être traduit par homme de fer.}.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item Que fait un chef de cuisine lorsqu'il soumet une mise à jour sur GitHub~? Un commit de cuisine\footnote{Un chef normal ferait appel à un commi\textbf{s} de cuisine.}. %bof
		\item C'est un stagiaire en développement qui utilise Git pour la première fois. Il veut récupérer la dernière version de la branche principale du projet. Donc on lui dit de faire un \textbf{git pull} mais rien n'y fait, ça ne marche pas. Au bout de longues minutes, son tuteur (manifestement excédé) jette un œil sur son PC et s'aperçoit qu'il a tapé~:\\\textbf{git \emoji{chicken}}\footnote{C'est la génération des personnes qui n'utilisent que des emojis. Je me demande aussi comment un fermier aurait interprété cette commande.}
        % Note de l'auteur : Vous n'imaginez pas à quel point c'est galère de rajouter des emoji dans un doc en LaTeX. Si vous galérez, il suffit de changer le compilo (LuaLatex), d'installer le package texlive-lualatex (Linux) et d'importer le package emoji.
        \item Pourquoi il n'y a jamais de problèmes pour configurer les imprimantes dans les aéroports\footnote{Source : l'impression du ticket d'embarquement se passe mieux que d'autres étapes en général}~? Parce qu'ils ont les bons pilotes\footnote{Des bons pilotes d'avion (on n'ira pas loin sans cela)}.
        \item Pourquoi Louis XVI ne travaille jamais sur les derniers commits d'une branche ? Parce qu'il travaille toujours à partir de `detached HEAD`\footnote{C'est quand on ne prend pas la dernière version (=HEAD) ou le dernier commit d'une branche)}.
        %\item...
	...
	\begin{enumerate}
        ...
        \item Que font deux BME280\footnote{Ce sont des capteurs de pression} dans une brasserie~? Ils se tapent des bars\footnote{C'est une unité de pression}~!
        \\\textit{\underline{Variante}~: Marche aussi avec des pêcheurs ou des poissonniers\footnote{Car le bar est aussi un poisson}.}
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\end{itemize}
    \item Yvon le cochon a été muté de porcherie\footnote{Même les cochons déménagent~!}. Après un long et éprouvant voyage, sa femme, Carrie la truie, prend de ses nouvelles et lui demande alors~: \guillemetleft Est-ce que tu es arrivé à bon porc~?~\guillemetright.
	\end{enumerate}
...
%		\item Pourquoi les grands-pères en Russie sont-ils aussi vieux~? Car ce sont des papis russes\footnote{Des papyrus. Hahahaha~!!!!}.
        ...
        \item Pourquoi les Libanais maîtrisent-ils bien Python~? Parce qu'ils savent gérer avec les \textit{taboulations}\footnote{Il est important de respecter les \textit{tabulations} en Python (il n'y a pas d'accolades comme certains langages bas niveau). Et dans un pays où le \textit{taboulé} est roi, c'est l'endroit parfait pour gérer cela~!}~!
	\end{enumerate}

	\chapter{Autres blagues}
	\emoji{underage} Attention, NSFW\footnote{Not Safe For Work}~! \\ \\
	Pensez à cacher ces quelques pages lors d'un repas avec la belle famille ou si vous voulez faire rire des enfants\footnote{Astuce~: ce chapitre et le suivant ont été stratégiquement placés à la fin du recueil. Ainsi, en milieu sensible, il vous suffira simplement d'arracher \sout{(et de détruire)} ces dernières pages.}. \\Et faites attention au public ciblé~!
	...
	\begin{enumerate}
        ...
        \item \texttt{Which is the most pleasant CLI\footnote{A command-line interface: an interface usable with a terminal and a keyboard (antonym: GUI: Graphical User Interface)} to work with? The CLI Toris\footnote{And it works with a single (magic) button! How convenient!}.}
	\end{enumerate}
	...
	\newpage
	\begin{enumerate}
		\item Pendant les attaques du 11-Septembre, pourquoi il aurait fallu filmer d’en bas~? Pour entendre les gens chanter «~\textit{It’s raining men}~».
        ...
        \item Quelle est la marque de terreau qui est si dangereuse que personne n'en veut~? Le terreau Riste\footnote{Pour des questions de référencement, je ne mettrai pas le mot que ça fait... Si quelqu'un en a, ça ne sentira pas bon pour lui.}.
	\end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Which cake is never ready on time? The choco-late cake.
    \end{enumerate}
